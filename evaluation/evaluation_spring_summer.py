...
### Reading in the new data scraped from TOAR here.

data = pd.read_csv('path_to_data.csv')

# set the 2 negative ozone values we have to small number.
...
def load_prepare_ukfrit_data_for_year_robust_features(data, last_day_of_training):
    ...
    training = TimeSeriesDataSet(
        ...
        allow_missing_timesteps=None # Seam8's edit
    )
...
# Need to make a dataset that selects just the spring or just summer data...

def load_prepare_ukfrit_spring_data(data, last_day_of_training):
    ...
    train_data_spring = train_data_raw.loc[(train_data_raw['datetime'].dt.month >= 3) & (train_data_raw['datetime'].dt.month <= 5)]
    train_data_rest_of_year = train_data_raw.loc[(train_data_raw['datetime'].dt.month < 3) | (train_data_raw['datetime'].dt.month > 5)]
    
    test_data_spring = test_data_raw.loc[(test_data_raw['datetime'].dt.month >= 3) & (test_data_raw['datetime'].dt.month <= 5)]
    test_data_rest_of_year = test_data_raw.loc[(test_data_raw['datetime'].dt.month < 3) | (test_data_raw['datetime'].dt.month > 5)]
    ...
    train_spring = TimeSeriesDataSet.from_dataset(training, train_data_spring, predict=False, stop_randomization=True)
    train_roy = TimeSeriesDataSet.from_dataset(training, train_data_rest_of_year, predict=False, stop_randomization=True)        
    
    test_spring = TimeSeriesDataSet.from_dataset(training, test_data_spring, predict=False, stop_randomization=True)
    test_roy = TimeSeriesDataSet.from_dataset(training, test_data_rest_of_year, predict=False, stop_randomization=True)    
    
    
    train_dataloader = training.to_dataloader(train=False, batch_size=batch_size, num_workers=num_workers, pin_memory=True) 
    val_dataloader = validation.to_dataloader(train=False, batch_size=batch_size * 10, num_workers=num_workers, pin_memory=True)
    test_dataloader = testing.to_dataloader(train=False, batch_size=batch_size * 10, num_workers=num_workers, pin_memory=True)
...
# Need to make a dataset that selects just summer data...

def load_prepare_ukfrit_summer_data(data, last_day_of_training):
    ...
    train_data_summer = train_data_raw.loc[(train_data_raw['datetime'].dt.month >= 6) & (train_data_raw['datetime'].dt.month <= 8)]
    train_data_rest_of_year = train_data_raw.loc[(train_data_raw['datetime'].dt.month < 6) | (train_data_raw['datetime'].dt.month > 8)]
    
    test_data_summer = test_data_raw.loc[(test_data_raw['datetime'].dt.month >= 6) & (test_data_raw['datetime'].dt.month <= 8)]
    test_data_rest_of_year = test_data_raw.loc[(test_data_raw['datetime'].dt.month < 6) | (test_data_raw['datetime'].dt.month > 8)]
    ...
    train_summer = TimeSeriesDataSet.from_dataset(training, train_data_summer, predict=False, stop_randomization=True)
    train_roy = TimeSeriesDataSet.from_dataset(training, train_data_rest_of_year, predict=False, stop_randomization=True)        
    
    test_summer = TimeSeriesDataSet.from_dataset(training, test_data_summer, predict=False, stop_randomization=True)
    test_roy = TimeSeriesDataSet.from_dataset(training, test_data_rest_of_year, predict=False, stop_randomization=True)    
    ...
    train_dataloader_summer = train_summer.to_dataloader(train=False, batch_size=batch_size * 10, num_workers=num_workers, pin_memory=True)
    train_dataloader_roy = train_roy.to_dataloader(train=False, batch_size=batch_size * 10, num_workers=num_workers, pin_memory=True)

    test_dataloader_summer = test_summer.to_dataloader(train=False, batch_size=batch_size * 10, num_workers=num_workers, pin_memory=True)
    test_dataloader_roy = test_roy.to_dataloader(train=False, batch_size=batch_size * 10, num_workers=num_workers, pin_memory=True)

    
    return training, train_dataloader, val_dataloader, test_dataloader, train_dataloader_summer, train_dataloader_roy, test_dataloader_summer, test_dataloader_roy    


# To generate results, the following code is run for different years, countries, and for spring and summer.

# e.g. test on 2008 spring and summer

# TFT checkpoints are different for each year
best_tft = TemporalFusionTransformer.load_from_checkpoint('path_to_checkpoint.ckpt')

country_name = 'UK, FR, IT, spring, 2008'

# load up the data into dataloaders, training, validation and testing
# feed this function data and the last day of training

training, train_dataloader, val_dataloader, test_dataloader, train_dataloader_spring, train_dataloader_roy, test_dataloader_spring, test_dataloader_roy  = load_prepare_ukfrit_spring_data(data, '2006-12-31')
...
ukfrit_actuals_just_spring_robust_log1p_ctrue, ukfrit_preds_just_spring_robust_log1p_ctrue = perform_model_predictions(test_dataloader_spring, best_tft)
# evaluate the performance of the TFT
print('**************** SCORES OF TFT HERE **********************')
scores = evaluate_metrics(ukfrit_actuals_just_spring_robust_log1p_ctrue[0::4, :], ukfrit_preds_just_spring_robust_log1p_ctrue[0::4, :])

percentile_scores = evaluate_metrics_percentile(ukfrit_actuals_just_spring_robust_log1p_ctrue, ukfrit_preds_just_spring_robust_log1p_ctrue, 90)

# save and make plots
scatter_plot_predictions_observations(ukfrit_actuals_just_spring_robust_log1p_ctrue[0::4, :], ukfrit_preds_just_spring_robust_log1p_ctrue[0::4, :], scores, country_name)

hexbin_plot_predictions_observations(ukfrit_actuals_just_spring_robust_log1p_ctrue[0::4, :], ukfrit_preds_just_spring_robust_log1p_ctrue[0::4, :], scores, country_name)

log_hexbin_plot_predictions_observations(ukfrit_actuals_just_spring_robust_log1p_ctrue[0::4, :], ukfrit_preds_just_spring_robust_log1p_ctrue[0::4, :], scores, country_name)